...
def get_block_histograms(directory, directory_output, directory_masks, n_patches, bins, query):
    ...
    for filename in os.scandir(directory):
        ...
        if f.endswith('.jpg'):
            ...
            if(query):
                ...
                mask = cv2.imread(directory + '/' + f_name + '_mask.png', cv2.IMREAD_GRAYSCALE)
                th, mask = cv2.threshold(mask, 128, 255, cv2.THRESH_BINARY)
                # add masks
