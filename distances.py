...
def query_measures_colour(hist_image, db_descriptors, distance_type, descriptor):
    """
    ...
    if descriptor == 'color':
        ...
        if np.isnan(hist_image.hist_gray).any() :
            idx = np.argwhere(np.isnan(hist_image.hist_gray))
            hist_image.hist_gray = np.delete(hist_image.hist_gray, idx)
            to_delete = True
        
        for key_db, img_db in db_descriptors.items():
            
            hist_gray_db = np.array(img_db.hist_gray)

            if to_delete:
                hist_gray_db = np.delete(hist_gray_db, idx)

            if distance_type == 'eucli':
                dist_gray = cv2.norm(hist_image.hist_gray, hist_gray_db, normType=cv2.NORM_L2)
            else:
                dist_gray = cv2.compareHist(hist_image.hist_gray, hist_gray_db, global_variables.argument_relations[distance_type])

            dists[key_db] = distances(dist_gray)
