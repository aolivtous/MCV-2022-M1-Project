...
# Function that uses Harris dectector to find corners in the image
def find_boxes_harris(image, f_name, printbox = False):
    height, width, _ = image.shape
    path_and_name = global_variables.dir_query + global_variables.dir_query_aux + f_name

    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    # HARRIS DETECTOR V2 - SubPixel
    ...
    # Delete the corners that are in the middle height of the image
    corners = np.delete(corners, np.where((corners[:,1] > ((height/2) - (height * 0.15))) & (corners[:,1] < ((height/2) + (height * 0.15)))), axis=0)
    # Delete the corners that are in the middle width of the image
    corners = np.delete(corners, np.where((corners[:,0] > ((width/2) - (width * 0.025))) & (corners[:,0] < ((width/2) + (width * 0.025)))), axis=0)
    # Delete the corners that are too close to the borders
    corners = np.delete(corners, np.where((corners[:,0] < (width * 0.025)) | (corners[:,0] > (width - (width * 0.025)))), axis=0)
    # Leave only the corners that are isolated
    ...
    cv2.imwrite(path_and_name + "_harris_sub_tb.jpg", image)

    # Dummy returns
    text_mask = np.zeros((height, width), dtype=np.uint8)
    text_box = [0, 0, 0+0, 0+0]
    bbox_output = [np.array([0, 0]),np.array([0, 0 + 0]),np.array([0 + 0, 0 + 0]),np.array([0 + 0, 0])]

    return text_box, text_mask, bbox_output


def find_boxes_harris_basic(image, f_name, printbox = False):
    height, width, _ = image.shape
    path_and_name = global_variables.dir_query + global_variables.dir_query_aux + f_name
    ...
    dst = cv2.dilate(dst, None)
    # # Treshold dct
    # ret, dst = cv2.threshold(dst, 0.20 * dst.max(), 255, 0)
    # # Dilate with a horizontal kernel
    # kernel = np.ones((1, 5), np.uint8)
    # dst_h = cv2.dilate(dst, kernel, iterations=1)
    # # Dilate with a vertical kernel
    # kernel = np.ones((5, 1), np.uint8)
    # dst_v = cv2.dilate(dst, kernel, iterations=1)
    # # Add both dilations
    # dst = dst_h + dst_v
    ...
    # Save the image with the boxes
    cv2.imwrite(path_and_name + "_harris_sub_lapl.jpg", image)
    # Save dst
    cv2.imwrite(path_and_name + "_harris_sub_dst.jpg", dst)
